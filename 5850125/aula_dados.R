...
ggplot(top_cursos_10, aes(x = curso_busca, y = mensalidade_max)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))  # rotacionar as labels do eixo x



# Questão 3

# Carregando a biblioteca dplyr para usar as funções filter, group_by e summarise
library(dplyr)

# Filtrando apenas os cursos tecnológicos na Paraíba
df_tec_paraiba <- dff %>% 
  filter(uf_busca == "PB" & grau == "Tecnológico")

# Agrupando os dados pelo nome do curso e somando as bolsas de cada tipo
top_bolsas_tec_paraiba <- df_tec_paraiba %>%
  group_by(curso_busca) %>%
...
print(top_bolsas_tec_paraiba$curso_busca[1:5])
...
## Cursos tecnologico da paraiba com maior total bolsas integrais cotas
...
df_grouped %>%
  ggplot(aes(x = turno_predominante, fill = indefinido_parcial_ampla_concorrencia)) +
